...
"SELECT users.name, SUM(pledges.amount) FROM users JOIN pledges on pledges.user_id = users.id GROUP BY users.name ORDER BY SUM(pledges.amount) ASC;"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM pledges JOIN projects ON pledges.project_id = projects.id WHERE projects.category = 'books';"
end
